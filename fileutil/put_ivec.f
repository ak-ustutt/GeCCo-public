*----------------------------------------------------------------------*
      subroutine put_ivec(fhand,ibuf,idxst,idxnd)
*----------------------------------------------------------------------*
*     write vector elements idxst to idxnd (in ibuf) to file fhand
*     integer version
*----------------------------------------------------------------------*
      implicit none
     ...
     &     allocate(recibuf(lenr))
      ...
      if (irecst.eq.irecnd) then
        ...
        if (ioffrec1.gt.0.or.idxrecl.lt.lenr) then
          ...
          read(un,rec=irecst,err=10) recibuf(1:lenr)
          goto 20
          ! error exception for uninitialized record
 10       recibuf(1:ioffrec1) = 0
          recibuf(idxrecl+1:lenr) = 0
 20       recibuf(ioffrec1+1:ioffrec1+nwrite) = ibuf(1:nwrite)
          write(un,rec=irecst) recibuf(1:lenr)
        else
          ! exactly one record to write
          write(un,rec=irecst) ibuf(1:lenr)
        end if
      else
        ! first record
        if (ioffrec1.eq.0) then
          ...
          ! handle "border"
          read(un,rec=irecst,err=11) recibuf(1:ioffrec1)
          goto 21
          ! handle uninitilized record
 11       recibuf(1:ioffrec1) = 0
 21       nwrite = lenr-ioffrec1
          recibuf(ioffrec1+1:ioffrec1+nwrite) = ibuf(1:nwrite)
          write(un,rec=irecst) recibuf(1:lenr)
          idxbfst = nwrite+1
        end if
        ...
        do irec = irecst+1, irecnd-1
          ...
          write(un,rec=irec) ibuf(idxbfst:idxbfnd)
          idxbfst = idxbfnd+1
        end do
        ...
        if (idxrecl.eq.lenr) then
          write(un,rec=irecnd) ibuf(idxbfst:idxbfnd)
        else
          ! handle "border"
          read(un,rec=irecnd,err=12) recibuf(1:lenr)
          goto 22
          ! handle uninitialized record
 12       recibuf(idxrecl+1:lenr) = 0
 22       recibuf(1:idxrecl) = ibuf(idxbfst:idxbfnd)
          write(un,rec=irecnd) recibuf(1:lenr)
        end if
      end if
