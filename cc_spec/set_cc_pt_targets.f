...
c      call set_dependency(form_ptdl0,op_tbar,tgt_info)
        ...
        if (r12fix.or.r12op.gt.0) then
          ...
          if (r12op.gt.0) then
            if (.not.screen) then
              ...
              call set_dependency(form_ptdl0,'Z2INT_R12',tgt_info)
              labels(8) = 'Vpx_formal'
              labels(9) = 'Vpx_formal^+'
              labels(10)  = 'Z2INT_R12'
              labels(11)  = 'Z2INT_R12^+'
     ...
     &       parameters,2,tgt_info)
        ...
        if (screen) then        
          ...
          labels(4) = op_r12
          labels(5) = op_r12//'^+'
          call set_rule(form_ptdl0,ttype_frm,INVARIANT,
     &         labels,5,1,
...
*----------------------------------------------------------------------*
     ...
     &              parameters,1,tgt_info)
      ...
      if (.not.screen.and.explicit)
     &     call set_dependency(fopt_ptde0,'Vpx-INTER',tgt_info)
      if (explicit)
     &     call set_dependency(fopt_ptde0,'DEF-Z2LIST',tgt_info)
      if (set_Rhxhh)
