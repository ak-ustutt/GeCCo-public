...
*----------------------------------------------------------------------*
     ...
     &     ivtx_tgt, ii, jj, ivtx, idx, iist, iind, iinc
      ...
      if (ntest.ge.100) then
        call write_title(lulog,wst_dbg_subr,'condense_merge_map')
        write(lulog,*) 'nvtx_tgt, ld_mmap: ',nvtx_tgt, ld_mmap
        if (nvtx_tgt.gt.1000 .or. ld_mmap.gt.1000) stop 'hilfe'
        write(lulog,*) 'input map'
        do ivtx_tgt = 1, nvtx_tgt
          write(lulog,'(x,i3,"<- op1 vtx ",10i4)')
     &         ivtx_tgt, mmap_in(1:ld_mmap,1,ivtx_tgt)
          write(lulog,'(6x," op2 vtx ",10i4)')
     &                   mmap_in(1:ld_mmap,2,ivtx_tgt)
        end do
        if (reverse) write(lulog,*) 'reverting Op1 and Op2'
      end if
      ...
      if (ntest.ge.100) then
        write(lulog,*) 'output map:'
        idx = 1
        do ivtx_tgt = 1, nvtx_tgt
          ivtx = mmap_out(idx)
          if (ivtx.gt.0) then
            write(lulog,'(3x,i3,"<-",i3," from op1: ",10i3)')
     &                        ivtx_tgt, ivtx,
     &                        mmap_out(idx+1:idx+ivtx)
          else
            write(lulog,'(3x,i3,"<-  0 from op1")') ivtx_tgt
          end if
          idx = idx + ivtx + 1
          ivtx = mmap_out(idx)
          if (ivtx.gt.0) then
            write(lulog,'(3x,i3,"<-",i3," from op2: ",10i3)')
     &                        ivtx_tgt, ivtx,
     &                        mmap_out(idx+1:idx+ivtx)
          else
            write(lulog,'(3x,i3,"<-  0 from op2")') ivtx_tgt
          end if
          idx = idx + ivtx + 1
        end do
c dbg
