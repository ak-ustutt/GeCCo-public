...
c dbg
c        print *,'current primitive arc: ',iarc_prm
c        print *,'ilist, len: ',ilist,len_list
c        print *,'current contr:'
...
c      if (update_ori.and.merge) then
c          ! delete ivtx2 in ivtx_ori
c        ivtx_ori(ivtx1) = ivtx_new
c        do ivtx = ivtx2, nvtx-1
c          ivtx_ori(ivtx) = ivtx_ori(ivtx+1)
c        end do
c      end if
      ...
      ! convert reordering array to cycles
c      call perm2cycl(ireo,icyc,nvtx)
