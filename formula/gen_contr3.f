...
c     &     occ_ol_vtx(:,:,:)
      if (ntest.ge.100) then
        ...
        write(lulog,*) '===================='
        write(lulog,*) 'nj_tgt = ',nj_tgt
        call prt_contr2(lulog,proto_main,op_info)
        call prt_contr3(lulog,proto_main,occ_vtx(1,1,nj_tgt+1))
      end if

      if (abs(proto_main%fac).lt.1d-20) then
        call quit(1,'gen_contr3',
...
cc                    call wrt_occ_n(lulog,occ_test,nj_tgt)
cc                    call wrt_occ_n(lulog,occ_vtx,nj_tgt)
cc dbg
c                    ok = ierr.eq.0.and.
