...
*----------------------------------------------------------------------*
     ...
     &     idx, isave, warning, error
      ...
      if (current%command.ne.command_set_target_init) then
        write(lulog,*) 'formula does not start with [INIT]'
        warning = warning+1
      end if
      if (associated(current%prev)) then
        write(lulog,*) 'there is a link to a previous item'// 
     ...
     &     command_internal,command_end_of_formula)
        ...
        if (.not.associated(current%contr)) then
          error = error + 1
          write(lulog,*) '[CONTR] without allocated data structure' 
        end if
        case(command_del_intermediate)
        if (.not.associated(current%label)) then
          error = error + 1
          write(lulog,*) '[DEL] without allocated label'
        end if
     ...
     &       '[NEW INTERMEDIATE] without allocated data structure'
        ...
        if (.not.associated(current%parent1)) then
          error = error + 1
          write(lulog,*) '[NEW INTERMEDIATE] without parent1 label'
        end if
        if (.not.associated(current%parent2)) then
          error = error + 1
          write(lulog,*) '[NEW INTERMEDIATE] without parent2 label'
        end if
     ...
     &      '[CONTR] without allocated data structure "bcontr"'
        ...
        case default
        error = error + 1
        write (lulog,*) 'unknown command ', current%command
        end select
     ...
     &          'there is still an allocated item after [END]'
        ...
        if (.not.associated(current%next)) then
          error = error + 1
          write(lulog,*) 'chain ends before [END]'
          exit
        end if
        if (.not.associated(current%next%prev,current)) then
          error = error + 1
          write(lulog,*) 'forward-backward inconsistency detected'
          idx = -1
          call print_form_item(lulog,idx,current,op_info)
          write(lulog,*) '... but current%next%prev is ...'
          idx = -1
          call print_form_item(lulog,idx,current%next%prev,op_info)
        end if
        current => current%next
        if (.not.associated(current%prev%next,current)) then
          error = error + 1
          write(lulog,*) 'backward-forward inconsistency detected'
          idx = -1
          call print_form_item(lulog,idx,current,op_info)
          write(lulog,*) '... but current%prev%next is ...'
          idx = -1
          call print_form_item(lulog,idx,current%prev%next,op_info)
        end if

        isave = isave+1
        if (isave.gt.maxlen) then
          error = error + 1
          write(lulog,*) 'Infinite loop? Or formula longer than ',maxlen
          exit
        end if
      end do
