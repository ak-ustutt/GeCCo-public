*----------------------------------------------------------------------*
      logical function contr_in_contr2(contra,contrb,op_info)
*----------------------------------------------------------------------*
...
c      allocate(extended_check(nvtxb))
     ...
     &        iblk_op1.eq.contrb%vertex(idx_b)%iblk_op) then
      ...
      if (narca.eq.0) contr_in_contr2 = .true.
...
c dbg
     ...
     &             iocc_xdn(2,occres(1:ngastp,1:2,jdx))
            ...
            contr_in_contr2 = contr_in_contr2 .and.
