*----------------------------------------------------------------------*
      subroutine gen_contr(form_list,proto,fix_vtx,occ_vtx,op_info)
*----------------------------------------------------------------------*
...
*
*     note: the routine only generates "pure" operator contractions
*           i.e. the result must be of operator type
*           density type expressions can be generated by inserting an
*           operator as "spacer" and taking the derivative wrt. to
*           that operator
*
*     andreas, june 2007
*
...
c     &     occ_ol_vtx(:,:,:)
     ...
     &     init, ok, must_connect(nvtx), reo
      integer ::
     &     nvtx_prev, jvtx, kvtx, iarc, narc_old, ieqvfac
      integer ::
     ...
     &     proto, proto_new
      logical, external ::
     &     next_part_connection, next_dist2
...
c dbg
     ...
     &         proto%arc(iarc)%occ_cnt(1,1).eq.-1)
     ...
     &                     iocc_nonzero(occ_conn(1:ngastp,1:2,jvtx))
              ...
              if (ok) then
                ...
                do jvtx = 1, nvtx_prev
                  ...
                  ok = .false.
                  do iarc = 1, narc_old
 1                  if (proto_new%arc(iarc)%link(1).eq.kvtx .and.
