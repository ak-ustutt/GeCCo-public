...
*----------------------------------------------------------------------*
     ...
     &     shw_arg = show_args
      ...
      key_loop: do
        ...
        if (shw_arg.and.associated(current%arg_h)) then
          curarg => current%arg_h
          write(fmtstr,'("(x,",i3,"x,a,i2,x,i4)")') 2*level+4
     ...
     &           write(lulog,'(10x,">",10i10)') curarg%val%ival
            if (associated(curarg%val%xval))
     &           write(lulog,'(10x,">",10f10.4)') curarg%val%xval
            if (associated(curarg%val%cval))
     &           write(lulog,'(10x,">",60a1)') curarg%val%cval
            if (associated(curarg%val%lval))
     &           write(lulog,'(10x,">",10l4)') curarg%val%lval
