      subroutine read_env_cfour(orb_info)
     ...
     &     irrtyp(:), fcreo(:)
      ...
      call file_init(ffintf,'cfour_gecco_ifc.dat',ftyp_sq_frm,0)
 ...
 99   continue
      ...
      if (.not.rd_intfile) then
        call warn(i_am,
     &     'could not read dumpfile name, trying "HF2"')
        intfile = 'HF2'
      end if
      if (.not.rd_nirr) then
      ...
      if (nirr.gt.2.and.nirr.ne.4.and.nirr.ne.8) then
        call warn(i_am,'Cfour''s nirrep is strange, trying to '
     &                  //'recover')
        if (nirr.eq.3) nirr=4
        if (nirr.gt.4) nirr=8
      end if
